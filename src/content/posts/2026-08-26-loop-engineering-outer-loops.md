...
이게 전부예요. 도구 호출이 없는 응답이 나오는 순간 루프가 끝나고 결과가 돌아와요.
...
안쪽 루프 자체는 고칠 수 없어요. 대신 감쌀 수는 있어요.
...
이 글에서는 "문 앞에 서 있는 사람"이라는 표현을 계속 쓸게요. 종류를 가르는 지점은 딱 하나예요. 거기에 누가 서 있느냐예요.

## 문 앞에 누가 서 있나

- **채점관** — `/goal`. 조건이 아직 참이 아니면 다시 들여보내요.
- **내가 쓴 스크립트** — Stop hook. 스크립트가 실패로 판정하면 다시 들여보내요.
- **시계** — `/loop`. 정해진 시간이 되면 다시 켜요.
- **셸** — Ralph 루프. 파일에 "끝"이라고 안 적혀 있으면 다시 불러요.
- **달력** — 스케줄과 cron. 다음 실행 시각이 되면 깨워요.
- **초인종** — Channels. 밖에서 사건이 들어오면 그때 움직여요.
...
- 측정 가능한 종료 상태 하나 — 테스트 결과, 빌드 종료 코드, 파일 개수, 빈 큐
- 증명 방법 — "`npm test`가 0으로 끝난다", "`git status`가 깨끗하다"
- 바뀌면 안 되는 제약 — "다른 테스트 파일은 수정하지 않는다"
...
세어보면 세 겹이에요. 맨 바깥에서는 달력이 매시간 깨워요. 그 안에서는 채점관이 리포트가 모두 처리될 때까지 붙잡아요.

가장 안쪽에는 워크트리 세 개가 병렬로 돌고, 각자 안쪽 루프를 따로 가져요. 심판 에이전트는 겹이 아니라 검증자예요. 안쪽의 결과를 채점해 바깥으로 올려보내는 자리죠.

이게 루프 안에 루프의 정체예요. 새 메커니즘은 아니에요. 같은 안쪽 루프를 무엇으로 감쌌는지만 달라요.
...
**맥락 낡음.** 제가 중간에 파일을 고쳐도 에이전트는 몰라요. 옛 가정 위에서 계속 달리죠. 루프가 길수록 위험해요.

**위험한 자율성.** 승인 없이 파괴적인 명령을 실행하는데, 앞의 셋이 시간을 태우는 문제라면 이건 되돌릴 수 없는 문제예요.

앞의 세 개에는 공통점이 있어요. 검증이 부실하면 모두 조용히 진행돼요.
...
계획하는 에이전트, 만드는 에이전트, 평가하는 에이전트를 셋으로 나눴어요. 서로 파일로 통신해요. 한쪽이 파일을 쓰면 다른 쪽이 읽는 방식이죠.
...
눈에 띄는 건 **검수를 세 번 돌렸는데 전체 비용의 8% 정도**라는 점이에요. 만드는 일은 비싸고 채점은 싸요. 채점하는 쪽을 따로 두는 게 사치는 아니라는 뜻이죠.
...
기준이 뚜렷한 일은 넘길 수 있어요. 테스트 통과 여부는 객관적이고, 성능 지표 달성도 숫자로 나오니까요.
...
**이해도 부채.** 루프가 코드를 빨리 만들수록 실제 코드와 제 이해 사이의 간격이 벌어져요. Osmani는 여기에 한 줄을 붙였어요. 이해는 무료가 아니에요.
...
루프는 멈추라고 말해주지 않아요. 멈추는 조건은 제가 써넣어야 하죠.
