...
이 글에서는 이 회차를 루프라고 부를게요. 실행 환경과 기본 도구·권한·상태를 제공하는 틀은 하네스, 여러 단계의 분기와 재진입 경로를 표현한 구조는 그래프라고 할게요.
...
안쪽 루프 자체를 고칠 수는 없어도 외부 검증을 연결할 수는 있어요. 에이전트가 "다 했습니다"라고 말하는 순간 외부 조건을 확인하고, 아직 끝나지 않았다면 새 회차로 이어주는 구조가 바깥 루프예요. 이 전환은 [Osmani 에세이가 인용한 Peter Steinberger](https://addyosmani.com/blog/loop-engineering/)의 한 문장에도 그대로 담겨 있어요. 링크는 인용을 실은 에세이로 이어지고, 원 게시물 자체는 이 글에서 확인하지 못했어요.

> [의역] 이제 코딩 에이전트에게 프롬프트를 치지 마세요. 에이전트에게 프롬프트를 치는 **루프를 설계하세요**.

Claude Code를 이끄는 Boris Cherny도 [같은 에세이에 실린 인용](https://addyosmani.com/blog/loop-engineering/)에서 자기 일을 비슷하게 설명했어요. Claude에게 직접 프롬프트를 치는 대신, 프롬프트를 치는 루프를 돌린다는 뜻이죠. 이 인용의 원 게시물도 이 글에서는 확인하지 못했어요.
...
조건을 문장으로 적어두면 매 턴이 끝날 때마다 evaluator(완료 여부를 판정하는 모델)가 판단하고, 결과는 ‘아직 아님’, ‘됐다’, ‘불가능’으로 나뉘어요. Claude API(프로그램으로 Claude를 호출하는 인터페이스)에서는 기본 판정 모델로 Haiku(빠른 확인에 쓰는 가벼운 모델)를 둔다고 설명해요. 다만 이 값은 provider(모델을 제공하는 서비스)·버전·설정에 따라 달라질 수 있으니 실행 전 공식 문서를 다시 확인해야 해요.

```text
/goal all tests in test/auth pass and the lint step is clean
```

`/goal`은 세션 범위에서 Stop hook을 감싼 기능이라 evaluator와 Stop hook이 같은 종료 지점에서 작동하지만 적용되는 수명은 달라요. evaluator가 도구를 직접 부르지 못하고 대화에 남은 출력만 보므로, 에이전트는 실제 명령과 결과를 먼저 기록해야 해요.

조건문에는 측정 가능한 종료 상태와 증명 방법을 적어야 하며, "`npm test`가 0으로 끝난다"처럼 명령과 종료 코드를 지정하고 "다른 테스트 파일은 수정하지 않는다"처럼 보호할 경계도 밝혀야 해요. 제자리걸음이 계속될 때만 "또는 20턴 뒤 중단" 같은 선택적 반복 상한을 더해 사람에게 제어권을 돌려줘요.
...
로컬 스케줄은 파일에 접근할 수 있지만 기계가 켜져 있어야 해요. 클라우드 루틴은 기계가 꺼져도 시작할 수 있지만 로컬 파일에는 접근하지 못해요.

실행 위치와 파일 접근 조건으로 세션 밖 일정을 고를 때는 클라우드 루틴의 최소 간격이 1시간으로 제한된다는 점도 함께 확인하고, 필요한 호출 빈도가 그 제한에 맞는지 먼저 따져야 해요. 기계·파일 접근 조건과 최소 간격은 모두 같은 문서의 스케줄 비교표에 있어요. [공식 도움말](https://code.claude.com/docs/en/scheduled-tasks)
...
위 그림은 하네스·그래프·루프의 관계를 그린 개념도로, 실행 가능한 설정이 아니라 세 요소가 어떻게 결합되는지 보여줘요. 아래 텍스트 블록은 같은 결합을 지시문 형태로 옮긴 예시예요.
...
일정 트리거인 `/schedule`(세션 밖 일정 실행을 지시하는 명령)이 작업을 시작하면 오케스트레이터는 세 격리 작업 공간에 에이전트를 배치해 각각의 해법을 탐색하게 하고, 각 작업 공간의 에이전트는 하네스가 허용한 파일·도구·권한만 사용해요. 여기서 워크플로(단계와 분기를 미리 정해 둔 작업 흐름)는 그 배치와 검토 순서를 가리켜요. 심판 에이전트는 세 결과를 검토하고, `goal`은 완료 조건을 확인해요.
...
이 관점에서 하네스 설계는 저장소 지식·추적 가능한 기록·완료 기준·검증과 복구 경계를 고정하는 일까지 포함해요. 아래 세 사례는 그 고정을 실제 운영에 옮긴 예로, 무엇을 하네스에 묶을지·작업 배분과 격리를 어디에 둘지·역할과 상태를 얼마나 잘게 나눌지를 차례로 보여줘요.

OpenAI의 [harness engineering](https://openai.com/index/harness-engineering/)은 앞의 조건을 다룬 인접 사례로 참고할 수 있어요. 배분과 격리를 한 단계 더 밀어붙인 쪽이 Symphony인데, issue tracker(작업 요청을 관리하는 도구)를 control plane(작업을 배분하고 상태를 관리하는 기준면)으로 삼고 작업별 workspace(작업 공간)를 격리하는 운영 사례예요. 다만 Symphony를 Graph Engineering의 구현이라고 부를 근거는 없어요. 역할과 상태를 더 잘게 나눈 쪽으로는 Steve Yegge의 [Gas Town](https://yegge.ai/essays/welcome-to-gas-town/)을 별도 사례로 볼 수 있어요.
...
[Osmani](https://addyosmani.com/blog/loop-engineering/)는 루프 설계에서 가장 유용한 구조적 이동을 "작성하는 에이전트와 검사하는 에이전트를 나누는 것"이라고 정리했어요. [Anthropic 하네스](https://www.anthropic.com/engineering/harness-design-long-running-apps)도 계획 담당자, 구현 담당자, 평가 담당자로 역할을 나눴어요. 구현 담당자가 파일을 쓰면 평가 담당자가 그 결과를 읽고 별도의 근거로 확인하는 구조예요.
...
이 세 결함은 "UI 개선이 필요해요" 같은 두루뭉술한 말이 아니라, 개발자가 추가 조사 없이 곧바로 손댈 수 있을 만큼 구체적이에요.
...
앞의 `/goal` 절에서 본 evaluator로 다시 돌아가 볼게요. 자율성을 주려면 멈춤도 시스템 계약 안에 넣어야 해요. 제가 관심 있게 보는 건 오류 이름보다 시스템이 다음 회차를 계속할지 취소할지를 구분하는 방식이에요. `/goal`의 evaluator는 인증 풀림, 크레딧 소진, 컨텍스트 압축 실패, 모델 사용 불가처럼 복구하지 않으면 반복될 오류를 사용자가 고쳐야 하는 실패로 판단하고, 이런 경우 Claude Code는 목표를 지워요. 호스트가 자격 증명을 자동으로 복구하면 목표가 유지될 수도 있어요. 반대로 속도 제한·서버 과부하처럼 잠시 뒤 사라질 실패는 목표를 유지해요.

백그라운드 작업이 남아 있으면 evaluator는 판정을 미뤄요. Claude Code는 30분이 지난 뒤 한 번 확인하고, 그다음에는 1시간, 이후에는 2시간 간격으로 확인해 불필요한 판정을 줄여요. [공식 문서](https://code.claude.com/docs/en/goal)

조건문은 4,000자까지 쓸 수 있고, 세션당 목표는 하나뿐이라 새 목표를 걸면 이전 목표가 교체돼요. [공식 문서](https://code.claude.com/docs/en/goal)

이 운영 경계는 사고를 줄이는 장치일 뿐이므로, 작업 완료는 테스트 결과나 로그로 따로 확인해야 해요.
...
제가 붙인 구성 요소 하나하나는 "모델이 이건 못 할 것"이라는 판단이라, 모델이 바뀌면 그 판단도 다시 재야 해요. [출처 요약] 모델이 좋아져도 필요한 안전장치가 모두 사라지는 건 아니고, 맡길 위치가 바뀐다고 설명해요.
...
Slack 기반 멀티 에이전트 시스템에서는 설계자와 구현자, 결정론 게이트(사람 판단 없이 통과 여부가 정해지는 검사), 리뷰 에이전트의 역할을 분리해 두었어요. 작업이 실패하면 전체를 처음부터 돌리지 않고 실패한 단계로 돌아가도록 구성했어요. 외부 동작은 사람 승인 뒤에 실행하도록 묶었어요.
...
`makeFixture()`는 입력 객체를 호출 가능한 함수로 만들고, 그 함수는 비동기 실행 결과를 `Promise`(결과가 나중에 채워지는 자바스크립트 객체)로 돌려줘요. `runVerification()`은 배열이나 시나리오 객체의 fixture를 실행해 각 결과의 `expectedExitCode`와 전체 `expectedOverallExitCode`를 확인해요. 아래 JSON은 이 입력 계약과 상태·단계를 함께 보여주는 설명용 메타데이터이고, 실행 함수 자체는 아니에요.
...
아래 상태 JSON은 현재 작업의 lease(한 작업자가 정해진 시각까지 그 작업을 혼자 붙잡는 권한)와 외부효과 상태를 보여주는 예시예요. `leaseUntil`의 `110`과 `timeoutAt`의 `130`은 초나 밀리초가 아니라 예제가 재현성을 위해 주입한 숫자 논리 시각이고, `version`은 claim 이후 저장소에 기록된 상태 갱신 횟수를 세요.
...
여기까지가 중복 처리와 선점을 막는 경계예요. 이제 그 선점을 시간으로 묶는 lease를 볼게요.

`running`에는 현재 작업자 `leaseOwner`와 lease의 만료 시각 `leaseUntil`을 두고, 상태의 `leaseToken`으로 이전 lease를 구분해요. `leaseOf()`가 반환하는 lease 객체는 상태의 `leaseToken`을 `token`이라는 필드로 담아요.

`worker-a`의 lease가 만료되면 `worker-b`가 한 번 회수하고 상태의 `leaseToken`을 증가시켜요. 그러면 회수된 `worker-a`의 lease 객체는 fencing(회수된 lease의 낡은 토큰을 거부하는 차단)에서 거부되고, 새 `token`을 받은 `worker-b`만 진행할 수 있어요.
...
주요 전이 함수의 입력과 반환 형태는 다음과 같아요. 구현 전문은 계약 테스트 파일에 있고, 여기에는 파일에서 확인할 수 있는 서명과 반환 형태만 옮겼어요.

| 함수 | 입력 | 반환 |
| --- | --- | --- |
| `moveToBuild(input, lease, now)` | 작업 식별자·lease 객체·현재 논리 시각 | 단계를 옮긴 상태 사본 |
| `moveToVerify(input, lease, now)` | 같은 셋 | 단계를 옮긴 상태 사본 |
| `runVerification(input, lease, now, fixtures)` | 같은 셋과 fixture 배열 또는 시나리오 객체 | `{ overallExitCode, attempts, results, state }` |
| `completeReview(input, lease, now, evidence)` | 같은 셋과 검토 근거 | `completedReport`가 채워진 상태 사본 |
| `approveRetry(input, owner, now, approval)` | 작업 식별자·작업자 이름·현재 논리 시각·승인 메타데이터 | `running`으로 되돌린 상태 사본 |
...
실행 기록에는 환경 정보를 먼저 남겨요. 저장소 루트는 `/Users/juneseok/repos/JSL107.github.io`이고, 실제 저장소 사본(checkout)의 커밋은 `git rev-parse HEAD`로 확인해요. `node --version`과 `uname -a`의 결과도 함께 기록해요.
...
예제 README에 적힌 실행 조건은 "Node.js 22 이상, 실행 디렉터리는 저장소 루트, 외부 provider·인증·네트워크·추가 패키지 불필요" 한 줄이에요. 다만 2026-09-04 재실행 때의 `node --version`·`uname -a`·커밋 해시의 실제 값은 기록에 남기지 않아 이번 글에서 확인하지 못했어요. 아래 명령의 `/Users/juneseok/repos/JSL107.github.io`는 작성자 환경의 checkout 경로이므로, 독자는 자기 저장소 루트로 바꿔서 실행하면 돼요.
...
| --- | --- | --- | --- |
| Loop Engineering | [직접 링크](https://addyosmani.com/blog/loop-engineering/) | 프롬프트를 직접 반복하기보다 프롬프트를 실행하는 루프를 설계한다는 관점, 작성자와 검증자 분리, 본문이 옮긴 Peter Steinberger·Boris Cherny 인용의 출처(두 사람의 원 게시물은 확인하지 못함) | 2026-09-04 |
...
| Keep Claude working toward a goal | [직접 링크](https://code.claude.com/docs/en/goal) | `/goal`·Stop hook의 완료 판정, evaluator, 세션 범위와 재시작, 백그라운드 작업 확인 간격 30분·1시간·2시간, 조건문 4,000자 상한, 세션당 목표 1개와 교체 규칙 | 2026-09-04 |
| Run prompts on a schedule | [직접 링크](https://code.claude.com/docs/en/scheduled-tasks) | `/loop`·cron·스케줄의 실행 조건, 세션 수명과 7일 만료, 클라우드 루틴과 로컬 스케줄의 기계·파일 접근 조건과 클라우드 최소 간격 1시간 | 2026-09-04 |
...
| Loop Engineering 로컬 재현 예제 구현·계약 테스트 | `examples/loop-engineering-reference-harness/contract.test.js` | 메모리 조건부 갱신 모사, 단계 전이, timeoutAt, 승인 전 외부효과 차단, fixture 결과. 이 블로그 저장소의 현재 Git 작업 디렉터리(working tree)에만 있고 공개 `main`에는 아직 없어 독자가 링크로 내려받을 수 없어요. 본문의 제품 동작 설명은 이 파일에 의존하지 않아요 | 2026-09-04 |
| Loop·하네스·그래프 개념도 SVG | `public/images/2026-09-loop-engineering/loop-harness-graph.svg` | 본문의 개념도. SVG(확장 가능한 벡터 그래픽) 파일도 같은 작업 디렉터리에만 있어 내려받을 공개 링크가 없고, 본문의 제품 동작 설명은 이 파일에 의존하지 않아요 | 2026-09-04 |
